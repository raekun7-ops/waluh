  ...
  state.mantri.forEach((mantri, mantriIndex) => {
    ...
    customers.forEach((customer, customerIndex) => {
      ...
      if (customerIndex === 0) {
        ...
        row.appendChild(createEditableInsuranceCell(mantri, mantriIndex, "amkkm", rowSpan, insurance));
        row.appendChild(createEditableInsuranceCell(mantri, mantriIndex, "brins", rowSpan, insurance));
...
function createEditableInsuranceCell(mantri, mantriIndex, type, rowSpan, autoValue) {
  ...
  input.type = "text";
  input.inputMode = "numeric";
  input.value = currentVal !== "" ? String(currentVal) : String(autoValue || 0);
  input.dataset.mantriIndex = String(mantriIndex);
  ...
  input.addEventListener("focus", () => {
    ...
    if (!isManual) input.value = String(autoValue || 0);
  });
...
function autoScrollToCenter() {
  function doScroll() {
    // Hanya scroll jika picker sudah tidak ada
