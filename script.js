...
async function exportReportImage() {
  ...
  try {
    if (_printBtn) { _printBtn.disabled = true; _printBtn.textContent = "⟳ Proses..."; }
    ...
    const filename = "laporan-" + (state.title || "margasari").toLowerCase().replace(/[^a-z0-9]+/g, "-") + ".png";

    // ── Deteksi iOS (Safari tidak support download attribute) ──
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
    const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);

    if (isIOS || isSafari) {
      // iOS/Safari: buka gambar di tab baru, user simpan manual (tap & hold → Simpan)
      ...
      if (win) {
        win.document.write(`
          ...
          <head>
            ...
            <style>
              ...
              p { color:#fff; font-family:sans-serif; font-size:14px; margin:16px 8px 8px;
        ...
        // Popup diblokir — tampilkan modal pratinjau dalam halaman
        showImagePreviewModal(canvas.toDataURL("image/png"), filename);
      }
    } else if (canvas.toBlob) {
      // Browser modern: unduh langsung via Blob URL
    ...
    console.error("exportReportImage error:", err);
    alert("Gagal membuat gambar: " + err.message);
  } finally {
    if (_printBtn) {
      setTimeout(function() {
        _printBtn.disabled = false;
        _printBtn.innerHTML = '<span aria-hidden="true">PNG</span> Gambar';
      }, 800);
    }
  }
}

// ── Modal pratinjau gambar (fallback jika popup diblokir) ──
function showImagePreviewModal(dataUrl, filename) {
  ...
  overlay.style.cssText = `
    ...
    overflow-y:auto;padding:20px 12px 32px;box-sizing:border-box;
  `;
  overlay.innerHTML = `
    <div style="width:100%;max-width:900px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
        <p style="margin:0;color:#fff;font-family:sans-serif;font-size:13px;opacity:0.8;">
          Tekan lama pada gambar → "Simpan Gambar" atau gunakan tombol di bawah.
        </p>
        <button id="closeImgModal" style="
          background:rgba(255,255,255,0.15);border:0;border-radius:8px;
          color:#fff;font-size:1.1rem;padding:6px 14px;cursor:pointer;
          font-family:sans-serif;font-weight:700;flex-shrink:0;margin-left:12px;">✕ Tutup</button>
      </div>
      <img src="${dataUrl}" alt="${filename}" style="width:100%;border-radius:8px;display:block;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
      <div style="text-align:center;margin-top:16px;">
        <a href="${dataUrl}" download="${filename}" style="
          ...
          box-shadow:0 6px 20px rgba(0,25,118,0.4);">
          ⬇ Unduh PNG
        </a>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  overlay.querySelector("#closeImgModal").addEventListener("click", () => overlay.remove());
...
function drawExportTable(ctx, x, y, columns, plans, tc) {
  ...
  plans.forEach((plan, mantriIndex) => {
    ...
    const amkkmVal = plan.mantri.amkkmOverride !== undefined ? plan.mantri.amkkmOverride : amkkmAuto;
    const brinsVal = plan.mantri.brinsOverride !== undefined ? plan.mantri.brinsOverride : brinsAuto;
    drawGroupCell(ctx, cx, gy, columns[totalIndex + 2], gh, amkkmVal ? formatRupiah(amkkmVal) : "", false, tc); cx += columns[totalIndex + 2];
    drawGroupCell(ctx, cx, gy, columns[totalIndex + 3], gh, brinsVal ? formatRupiah(brinsVal) : "", false, tc); cx += columns[totalIndex + 3];
    (state.dhColumns || []).forEach((dhName, dhIdx) => {
